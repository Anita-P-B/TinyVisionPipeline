...
from tiny_vision_pipeline.utils.visualization_utils import plot_predictions
...
def predict(args):
    ...
    # load model configs
    with open(os.path.join(model_dir, 'train_config.json'), 'r') as f:
    ...
    if args.test and args.custom_test is not None:
    ...
    if args.test:
        ...
        # select 9 random samples for prediction
...
parser.add_argument('--test', action='store_true', default=None,
                    help="Test the trained model on the CIFAR-10 test dataset")
parser.add_argument('--custom_test', type=str, default=None,
                    help="Test the trained model on custom images from the specified folder path")
parser.add_argument('--model_path', type=str,
                    ...
                    # Default model path for quick testing (replace with your own when needed)
